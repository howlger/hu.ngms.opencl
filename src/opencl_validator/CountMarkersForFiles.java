...
public class CountMarkersForFiles implements IEditorActionDelegate {
	...
	public void run(IAction action) {
		TreeSelection selection = OpenCLMarkerFactory.getTreeSelection();
		IResource resource;
		if (selection.getFirstElement() instanceof IOpenable) {
			...
			List<IMarker> markers = OpenCLMarkerFactory.findAllMarkers(resource);
			MessageDialog dialog = new MessageDialog(Activator.getShell(), "Marker Count", null, markers.size()
					+ " marker(s)", MessageDialog.INFORMATION, new String[] { "OK" }, 0);
			dialog.open();
		}
